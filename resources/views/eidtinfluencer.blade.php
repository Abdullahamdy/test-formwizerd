...
<body>
    ...
    <div class="container">
        <div class="row">
            <form method="post" action="{{route('updateinfluencer',$influencer->id)}}">
                @csrf
                <div class="col-lg-4">
                    <div class="form-group">
                        <label for="exampleInputName">Name</label>
                        <input type="text" name="name"  value="{{$influencer->name}}" class="form-control" id="exampleInputName1"
                            ...
                            <ul id="pac">
                                @foreach ($classifications as $classification)
                                    <li style="display: list-item;"><input name="classification_ids[]"
                                        @php
                                           $arry =  explode(',',$influencer->classification_ids)
                                        @endphp  
                                    
                                     {{in_array( $classification->id,$arry ) ? 'checked' : ''}} value="{{ $classification->id }}" type="checkbox"><label
